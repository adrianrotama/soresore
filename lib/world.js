...
export function cellSurfaceWorld(world, gx, gz) {
  ...
  // Decorations authored at integer cell indices (n) are *placed* at the cell center
  // by `gridToWorld`, but if we sample slope/stair height at (n,n) we’re sampling the
  // cell corner (fraction 0), which can resolve to the ramp’s highest edge depending
  // on rotation. Sample at the same center point we place at.
  const fx = Math.abs(gx - Math.round(gx));
  const fz = Math.abs(gz - Math.round(gz));
  const sampleGx = fx < 1e-6 ? gx + 0.5 : gx;
  const sampleGz = fz < 1e-6 ? gz + 0.5 : gz;

  return [wx, surfaceYAt(world, sampleGx, sampleGz), wz];
}
