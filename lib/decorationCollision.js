...
export function listDecorationCollisionFootprints(world) {
  ...
  for (const landmark of world?.landmarks ?? []) {
    ...
    const anchor = landmarkGridAnchor(world, landmark);
    const keyBase = `landmark-${landmark.kind}-${anchor.gx}-${anchor.gz}`;
    boxes.push({
      key: keyBase,
      ...resolveDecorationDebugBox(world, anchor, collision),
    });

    for (const cellKeyStr of cellsForDecoration(anchor, collision)) {
      const [ix, iz] = cellKeyStr.split(",").map(Number);
      const gx = ix + 0.5;
      const gz = iz + 0.5;
      const [cx, , cz] = gridToWorld(gx, gz, world.origin);
